...
export class WorkoutPlayer {
  ...
  start(name, intervals) { const [work, rest] = intervals; this.state = { name, exercises: WORKOUTS[name].exercises, work, rest, phase: 'work', round: 1, index: 0, remaining: work, paused: false }; const exercise = this.state.exercises[0]; this.speak(`Welcome to Project Under 100. ${name}. Round one. ${exercise}. ${EXERCISE_GUIDANCE[exercise]?.cue || 'Move with control.'} Three, two, one, go.`); this.render(); this.timerId = window.setInterval(() => this.tick(), 1000); }
  ...
  advance() { const s = this.state; if (s.phase === 'work') { s.phase = 'rest'; s.remaining = s.rest; this.speak(`Recover. Next: ${s.exercises[(s.index + 1) % s.exercises.length]}.`); return; } if (s.index === s.exercises.length - 1) { if (s.round === 2) return this.complete(); s.round += 1; s.index = 0; this.speak(`Round two. ${s.exercises[0]}. ${EXERCISE_GUIDANCE[s.exercises[0]]?.cue || 'Go.'}`); } else { s.index += 1; const exercise = s.exercises[s.index]; this.speak(`${exercise}. ${EXERCISE_GUIDANCE[exercise]?.cue || 'Move with control.'}`); } s.phase = 'work'; s.remaining = s.work; }
