...
export async function uploadState(state) { const user = await currentUser(); if (!user) throw new Error('Sign in first to back up your progress.'); const { error } = await client.from('fitness_states').upsert({ user_id: user.id, state: withoutPhotos(state), updated_at: new Date().toISOString() }); if (error) throw error; }
